...
export class UpdateService {
  ...
  constructor(appRef: ApplicationRef, updates: SwUpdate, confirmationService: ConfirmationService,
    ...
    if (!updates.isEnabled) {
      toastService.showWarn('Updates not available');
      return;
    }
    ...
    updates.unrecoverable.subscribe(() => {
      toastService.showError('Update failed: unrecoverable');
      setTimeout(() => {
    ...
    updates.versionUpdates.subscribe(vu => {
      if (vu.type === 'VERSION_INSTALLATION_FAILED') {
        toastService.showError('Installation of update failed');
        setTimeout(() => {
          window.location.reload();
        }, 1000);
        return;
      ...
      if (vu.type === 'VERSION_DETECTED') {
        toastService.showInfo('New version detected');
        return;
      }
      if (vu.type === 'VERSION_READY') {
        confirmationService.confirm('Update downloaded. Install now?').subscribe(r => {
          if (r) {
